...
def fully_connected_backward(W, b, X, output_grad):
    ...
    # (dL/dB) = (dL/dY)
    b_grad = output_grad.T @ np.ones(X.shape[0])
...
def train(x_train, target_train, x_val=None, target_val=None, epoch_count=100, learning_rate=0.04):
    ...
    for epoch in range(epoch_count):
        ...
        dh, d_w3, d_b3 = fully_connected_backward(W3, b3, h, dw)
        dv = relu_backward(v, dh)
        dg, d_w2, d_b2 = fully_connected_backward(W2, b2, u, dv)
        du = relu_backward(g, dg)
        dx, d_w1, d_b1 = fully_connected_backward(W1, b1, x_train, du)
